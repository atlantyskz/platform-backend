...
class CloneController:
    ...
    def __init__(self, session: AsyncSession):
        ...
        self.clone_repo = repositories.CloneRepository(session)
    ...
    async def create_clone(
        ...
        async with self.session.begin():
            try:
                ...
                agreement_uploaded = await self.minio_uploader.upload_single_file(agreement_video_data,
                                                                                  agreement_video_key)
                ...
                raise exceptions.BadRequestException(f"Error creating clone: {str(e)}")
    ...
    async def get_request_by_id(self, user_id: int, id):
        async with self.session.begin():
            ...
            if request.user_id != user_id:
                raise exceptions.BadRequestException('User not found')
    ...
    async def stream_clone_video(self, user_id: int, clone_id: int, video_type: str) -> StreamingResponse:
        async with self.session.begin():
            ...
            if not clone_request:
                raise exceptions.NotFoundException("Clone request not found")
            ...
            if video_type == 'agreement':
                ...
                raise exceptions.BadRequestException("Invalid video type specified")

            video_stream = self.minio_uploader.get_file(video_key)
            if video_stream is None:
                raise exceptions.NotFoundException("Video not found in storage")
