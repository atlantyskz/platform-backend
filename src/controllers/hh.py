...
class HHController:
    ...
    async def fetch_resume_details(self, user_id: int, resume_id: str) -> dict:
        ...
        Используется экспоненциальный бэкофф и кэширование.
        """
        # Если резюме уже получено ранее, возвращаем его из кэша.
        ...
        while retry_count < max_retries:
            ...
            if response.status_code == 200:
                data = response.json()
                _resume_cache[resume_id] = data  # сохраняем в кэш
                return data
            elif response.status_code == 429:
                retry_after = response.headers.get("Retry-After")
                delay = int(retry_after) if retry_after and retry_after.isdigit() else (2 ** retry_count)
    ...
    async def analyze_vacancy_applicants(self, session_id: str, user_id: int, vacancy_id: int) -> dict:
        """
        Анализ вакансии: получение текста вакансии, резюме кандидатов и создание фоновых задач для их анализа.
        Интегрирована обработка rate limiting для получения деталей резюме.
        """
        ...
        async with self.session.begin():
            ...
            # Параллельно получаем детали каждого резюме с обработкой rate limiting
            ...
            # Обработка каждого резюме и создание фоновых задач
